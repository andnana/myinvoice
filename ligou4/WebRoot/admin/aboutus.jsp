...
<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c" %>
        ...
        <div class="row-fluid">
         ...
         <a class="commoninfoformlist" style="cursor:pointer;color:blue;">信息列表</a>
			...
			<form class="form-horizontal" action="<%=basePath %>saveAboutUsServlet" id = "commoninfoform" method="post">
			...
			<div id="commoninfoformlist">
					...
					<c:forEach var="aboutUsText" items="${aboutUsTextList}">					 
										<tr>
									<td>${aboutUsText.content }</td>
										
										
											<td>
											
											<c:choose>
											<c:when test="${aboutUsText.active == 1 }">
											已启用
											</c:when>
											<c:otherwise>
												已禁用
											...
											<td>
											<a href="deleteAboutUsServlet?id=${aboutUsText.id }">删除</a>
												|<a href="updateAboutUsActiveServlet?id=${aboutUsText.id }">启用</a>
