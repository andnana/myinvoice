...
<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c" %>
		...
		<tr><th>id</th><th>顾客</th><th>销售人员</th><th>是否完成支付</th><th>附加信息</th><th>税前总金额</th><th>税</th><th>税后总金额</th><th>日期</th><th>操作</th></tr>
		<s:iterator var="deal" value="dealList"> 
		<tr><td><s:property value="id"/></td>
		<td>
		 <s:if test="customer.customername.length()>6">  
     <s:property value="customer.customername.substring(0,6)+'...'" />  
 </s:if>  
 ...
 <s:else>  
      <s:property value="customer.customername" />  
		...
		<s:if test="salePerson.salepersonname.length()>6">  
     <s:property value="salePerson.salepersonname.substring(0,6)+'...'" />  
 </s:if>  

 <s:else>  
      <s:property value="salePerson.salepersonname" />  
 </s:else>
		...
		<s:if test="note.length()>6">  
     <s:property value="note.substring(0,6)+'...'" />  
 </s:if>  
