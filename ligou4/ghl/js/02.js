...
$("#findsaleperson").click(function(){
	...
	$("table.gridtable").on("click","td:nth-child(6)>a",function(){
		if($findwho=="customer"){
...
//		});
	...
	$("form[action^='adddeal'] :submit").click(function(event){
		
		if($addproduct2Index < 0){
			alert("请添加至少一种产品");
		...
		if($("form[action^='adddeal'] input[id^='long']").val()== "" || $("form[action^='adddeal'] input[id^='width']").val() == ""){
			alert("请添加产品长度或宽度");
			event.preventDefault();
		}
		$product2select0val = $("#product2select0").val();
		if(parseInt($product2select0val) == 0){
			alert("请添加至少一种产品");
			event.preventDefault();
		}
		if($("form[action^='adddeal'] input[id^='amount']").val()== ""){
			alert("请重复添加一次长度宽度");
			event.preventDefault();
		}
...
function longblur(longid){
	...
	$widthval = parseInt($("#"+longid).parent().next().find("input").val());
		if($longval>0 && $widthval>0){
			...
			if($quantityInput.parent().next().find("input").val() == ""){
				alert("请先添加产品，再添尺寸");
			}else{ 
